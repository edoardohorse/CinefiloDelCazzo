...
const CardFilm = ({film}: TCardFilmProps) => {
	...
	return (
		<Card type="ambient" key={film.id} className={'card-film'} onClick={onClickCard}>
			<Fragment>
				...
				/>
				<Card.Cell className={'card-film-cell'} subtitle={<CardDate film={film}/>}>{film.name}</Card.Cell>
			</Fragment>
		</Card>
	)
}
